...
import { CloseIcon, ChevronLeftIcon, ChevronRightIcon, CopyIcon, SparklesIcon, VideoCameraIcon, DownloadIcon, WandIcon, WarningIcon } from './icons';
...
const ActionButtons: React.FC<ActionButtonsProps> = ({ image, onRecreate, onAnimate, onEnhance, isPreparingAnimation, settings, currentUser, isFloating }) => {
...
const MetadataPanel: React.FC<MetadataPanelProps> = ({ image, onRecreate, isPreparingAnimation, onAnimate, onEnhance, onKeywordClick, settings, currentUser, onTogglePublicStatus, onCopyPrompt, isCopied, isVisible, onRetryAnalysis }) => {
      ...
      {image.recreationPrompt && (
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <ActionButtons {...{image, onRecreate, onAnimate, onEnhance, isPreparingAnimation, settings, currentUser, isFloating: false}} />
          </div>
          <div className="relative">
...
const ImageViewer: React.FC<ImageViewerProps> = ({
  ...
  const slideshowImages = useMemo(() => {
    return similarImages.length > 0 ? similarImages : [initialImage];
  }, [similarImages, initialImage]);
  ...
  const startSlideshowLoop = useCallback(() => {
    if (slideshowImages.length <= 1 || isLoading || error || isVideo) return;
    nextImage();
    const randomDelay = 4000 + Math.random() * 6000;
    slideshowTimerRef.current = window.setTimeout(startSlideshowLoop, randomDelay);
  }, [nextImage, slideshowImages.length, isLoading, error, isVideo]);

  const resetInactivityTimer = useCallback(() => {
    if (slideshowTimerRef.current) clearTimeout(slideshowTimerRef.current);
    if (isLoading || isVideo) return;
    slideshowTimerRef.current = window.setTimeout(startSlideshowLoop, 8000);
  }, [startSlideshowLoop, isLoading, isVideo]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="relative flex items-center justify-center w-full h-full max-h-[85vh] max-w-[85vw]">
            ...
            {currentImage && <MetadataPanel image={currentImage} onRecreate={handleRecreate} isPreparingAnimation={isPreparingAnimation} onAnimate={handleAnimateClick} onEnhance={handleEnhance} onKeywordClick={onKeywordClick} onTogglePublicStatus={onTogglePublicStatus} onCopyPrompt={handleCopyPrompt} isCopied={isCopied} settings={settings} currentUser={currentUser} isVisible={isDetailsVisible} onRetryAnalysis={onRetryAnalysis} />}

             {!isVideo && currentImage && !isDetailsVisible && (
                 <ActionButtons {...{image: currentImage, onRecreate: handleRecreate, onAnimate: handleAnimateClick, onEnhance: handleEnhance, isPreparingAnimation, settings, currentUser, isFloating: true}} />
             )}
        ...
        {!isVideo && slideshowImages.length > 1 && !isLoading && (
            <>
                ...
                >
                 ...
                 <div className={`absolute top-5 left-1/2 -translate-x-1/2 p-2 bg-black/50 rounded-full text-xs text-white/90 z-20 transition-opacity duration-300 ${isDetailsVisible ? 'opacity-100' : 'opacity-0'}`}>
                    <p>Auto-slideshow of similar images</p>
                </div>
            </>
        )}
      </div>
