...
export function PerformanceStats({ statsHistory, timeRange }: PerformanceStatsProps) {
    ...
    return (
        <div className="bg-neutral-900/50 border border-white/10 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Activity / Requests Chart (Using timestamp density as proxy for request activity) */}
                <div className="bg-black/20 rounded-xl p-4 border border-white/5">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2 text-neutral-400">
                        ...
                        <div className="absolute inset-0 z-10 p-1">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={filteredStats}>
                                    ...
                                    <YAxis hide />
                                    {/* Using a constant value 1 for each point effectively visualizes density of requests over time on the X axis */}
                                    <Area
                                        type="step"
                                        dataKey={() => 1}
                                        stroke="#8b5cf6"
                                    ...
                                    <Tooltip
                                        ...
                                        formatter={() => ['Active', 'Status']}
                                        labelFormatter={(label) => new Date(filteredStats[label]?.timestamp).toLocaleTimeString()}
                                    />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
